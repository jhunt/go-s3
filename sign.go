...
func signableQueryString(s string) []byte {
	if s == "" {
		return []byte{}
	}
...
func signableHeaders(req *http.Request) ([]byte, []byte) {
...
func (c *Client) signature(req *http.Request, raw []byte) string {
	...
	*/
	...
	headers, hsig := signableHeaders(req)
	canon := sha256.New()
	canon.Write([]byte(req.Method))
	canon.Write([]byte("\n"))
	canon.Write(uriencode(req.URL.Path, false))
	canon.Write([]byte("\n"))
	canon.Write(signableQueryString(req.URL.RawQuery))
	canon.Write([]byte("\n"))
	canon.Write(hsig)
	canon.Write([]byte("\n"))
	...
	//	req.Method, string(uriencode(req.URL.Path, false)), string(signableQueryString(req.URL.RawQuery)), string(hsig), hashed)
